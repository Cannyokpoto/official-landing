  ...
  return (
    <div className="flex flex-col items-center h-auto gap-5 py-5 large:w-100vw small:w-90vw text-15px bg-vogueWhite">
      ...
      <div className="flex flex-col items-center h-auto gap-2 w-100">
        <h1 className="font-bold text-landingBlue large:text-25px small:text-20px">
          EXPLORE OUR COURSES
        </h1>
        <p className="text-center small:px-1 large:px-0">Click on a course to see the course content.</p>
      </div>

      <div className="flex flex-col h-auto gap-3 text-landingBlue large:w-50 text-15px small:w-100 small:px-1">

        {
            courses.map((course, i)=>
                <Link
                    to={`/${course.path}`}
                    key={i}
                    className="flex items-center justify-between h-auto p-1 font-bold border border-landingBlue w-100 rounded-4 hover:text-vogueWhite hover:bg-landingBlue"
                >
                    {course.name}
